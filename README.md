...
1. **the System's Functionality**: Produce all planned payments (from the date of the first payment) for every single arrangement in a file provided.
...
- **Enhance Operational Efficiency**: Automating the calculation of payment schedules reduces manual errors and saves time for Credit Management Company's agents.
...
The project includes a live application that provides an interactive interface for users to explore and interact with their payment arrangements data.
...
- **`data/`**: Contains the data files.
  - `CaseStudy Data.csv`: The source dataset containing anonymized arrangements for testing.
